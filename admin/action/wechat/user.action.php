...
class UserAction extends AdminAction {
  	 ...
  	 public function index() {
		...
		$users=M('wechat/template','list',$data);
    		// $total=M('wechat/template','count',$data);

        // $cats=M('wechat/cat','all');

    		foreach ($users as $k => $template) {
    			
    			$users[$k]['action'][]=array('href'=>admin_url('wechat/template/update/'.$template['template_id']),'text'=>L('text_edit'));
    		}
